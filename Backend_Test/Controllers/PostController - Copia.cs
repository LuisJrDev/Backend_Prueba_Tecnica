...
namespace MiWebAPI.Controllers
{
    ...
    public class PostController : ControllerBase
    {
        ...
        public async Task<IActionResult> Listar()
        {
            try
            {
                var lista = await _postRepository.Listar();
                return Ok(lista);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Crear([FromBody] Post post)
        {
            ...
            try
            {
                ...
                if (resultado)
                {
                    return CreatedAtAction(nameof(Detalles), new { id = post.IdPost }, post);
                }
        ...
        [HttpPut]
        public async Task<IActionResult> Editar([FromBody] Post post)
        {
            if (post == null)
            {
                return BadRequest("El objeto post no puede ser null");
            ...
            try
            {
                ...
                return NotFound($"Post con ID {post.IdPost} no encontrado o no se pudo editar");
            }
            catch (Exception ex)
            {
